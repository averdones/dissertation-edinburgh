function [best_lelbow_global, particlescart_global, weights, elapsed_time] = particle_filter_lelbow(...
    mergedpcl, best_lshoulder, bone_dist, n_particles, init_noise, move_noise, obs_noise, ...
    az_init, el_init, inf_lim_az, sup_lim_az, inf_lim_el, sup_lim_el, ...
    min_part, max_part, cylinder_elbow, verbose)
%{
    Particle filter of the left elbow.

    Inputs:
    - mergedpcl: merged point cloud with the four kinects.
    - best_lshoulder: best particles of left shoulder to use as starting particles for the elbow.
...
if nargin < 10 || isempty(inf_lim_az)
     inf_lim_az = -pi/4;  % 1.2 degrees approx.
end
...
% Position of the left elbow at every frame
elbow = best_lshoulder;
...
[~,ind_best_lelbow] = max(weights, [], 1);
best_lelbow = zeros(n_dim+1, n_frames);
for f = 1:n_frames
    best_lelbow(:,f) = particlescart(:,ind_best_lelbow(f),f);
end
...
best_lelbow_global = best_lelbow + elbow;
