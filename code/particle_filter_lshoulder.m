function [best_lshoulder_global, particlescart_global, weights, elapsed_time] = particle_filter_lshoulder(...
...
%{
    Particle filter of the left shoulder.
...
if nargin < 12 || isempty(inf_lim_az)
     inf_lim_az = -pi/6;  % 1.2 degrees approx.
end
...
init = unifrnd(repmat(([inf_lim_az + pi/12 inf_lim_el + pi/6])', 1, n_particles), ...
    repmat(([sup_lim_az sup_lim_el - pi/6])', 1, n_particles));

% Restrict azimuth between -pi/2 and pi/3 anmd
...
%     drawn_particles = datasample(particles(:,:,t-1),n_particles,2,'Weights',weights(:,t-1));
    ...
    for p = 1:n_particles
        if mod(p, 10) == 0 && verbose
            fprintf('Left shoulder particle - %d\n', p);
        end
...
[~,ind_best_lshoulder] = max(weights, [], 1);
best_lshoulder = zeros(n_dim+1, n_frames);
for f = 1:n_frames
    best_lshoulder(:,f) = particlescart(:,ind_best_lshoulder(f),f);
end
...
best_lshoulder_global = best_lshoulder + shoulder;
